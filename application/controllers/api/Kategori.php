...
class Kategori extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model(array('api_category_model'));
	...
	{
		$data = $this->api_category_model->index();
	...
	{
		$data = array(
			'nama_category' => $this->input->post('name'), 
			'status' => $this->input->post('status')
		);
		if (!empty($this->input->post('id_category'))) {
			$id_category = $this->api_category_model->update($data);
		} else {
			$id_category = $this->api_category_model->create($data);
		}
	...
	{
		$data = $this->api_category_model->getById($id);
	...
	{
		$data = $this->api_category_model->destroy($id);
		$this->output->set_content_type('application/json')->set_output(json_encode(['status' => true]));
	}
}
