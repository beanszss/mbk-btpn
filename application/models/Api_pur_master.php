...
class Api_pur_master extends CI_Model
{
    public function index()
    {
       ...
       $primaryKey = 'id_pur';
       $columns = array(
           array('db' => '`mw`.`id_pur`', 'dt' => 'id_pur', 'field' => 'id_pur'),
           ...
           array('db' => '`mw`.`id_pur`', 'dt' => 'actions', 'field' => 'id_pur'),
        //    array('db' => 'id_device', 'dt' => 'id_device', 'field' => 'id_device'),
        //    array('db' => 'id_device', 'dt' => 'id_device', 'field' => 'id_device'),
       );
       $sql_details = $this->configDB();
       $joinQuery = "FROM `master_pur` AS `mw` 
    ...
    public function warranty()
    {
        ...
        // $where = "'`mw`.`status_allocated`' = 'BACKUP IT'";
        $sql_details = $this->configDB();
        $joinQuery = "FROM `master_pur` AS `mw` 
        LEFT JOIN `lob` AS `lb` ON (`lb`.`id_lob` = `mw`.`id_lob`)
        LEFT JOIN `tb_service` AS `sc` ON (`sc`.`id_service` = `mw`.`id_service`)";
